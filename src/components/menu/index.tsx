  ...
  useEffect(() => {
    ...
    const handler = (event: KeyboardEvent) => {
      ...
      if (!open) {
        ...
        if (event.key === 'ArrowDown') {
          const currentMenuItemIndex = menuItems.findIndex(
            (item) => item.id === activedescendantRef.current,
          )
          if (currentMenuItemIndex === -1) {
            return
          }
          const nextMenuItemIndex = currentMenuItemIndex + 1
          if (nextMenuItemIndex >= menuItems.length) {
            return
          }

          setActivedescendant(menuItems[nextMenuItemIndex]?.id)
        } else if (event.key === 'ArrowUp') {
          const currentMenuItemIndex = menuItems.findIndex(
            (item) => item.id === activedescendantRef.current,
          )
          ...
          const previousMenuItemIndex = currentMenuItemIndex - 1
          if (previousMenuItemIndex < 0) {
            return
          }
          setActivedescendant(menuItems[previousMenuItemIndex]?.id)
        }
      }
    }
